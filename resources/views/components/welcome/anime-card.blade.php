    ...
    >
        ...
        <div class="relative aspect-[3/4] overflow-hidden">
            ...
            <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-3">
                <flux:text size="sm" class="text-white font-bold">View Details →</flux:text>
            </div>

            <!-- Rank Badge (if provided) -->
            @if ($rank)
                <div class="absolute top-2 left-2 w-8 h-8 rounded-lg flex items-center justify-center font-black text-sm border-2 border-border-brutal {{ $color }}">
                    #{{ $rank }}
                </div>
            @endif
            ...
            @if ($status)
                <div class="absolute top-2 left-2 px-2 py-1 rounded-lg font-bold text-xs border-2 border-border-brutal {{ $color }}">
                    {{ $status }}
                </div>
            @endif
        </div>
